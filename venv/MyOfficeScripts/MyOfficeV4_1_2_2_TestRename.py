...
def extract_txt_doc(path,folderName,mydirs_,lena):
    ...
    if is_changed:
        ...
        document_xls_input.saveAs((mydirs_[0] + "\\"+"№_"+str(lena)+"_"+last_name+"_"+first_name+"_Ошибка.xlsx").encode('utf-8'))
        os.remove(path)
    else:
...
def main_(worker, folderName,mydirs_,date_end):
    ...
    write_table(all_str_lst, worker,date_end,n_rows+1) #Массив передастся в упорядоченном виде скорее всего
