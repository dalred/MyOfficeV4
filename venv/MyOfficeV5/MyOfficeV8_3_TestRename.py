...
def extract_txt_doc(path, folderName, mydirs_, lena,log,workstatuspanel):
    try:
        ...
        raise Exception(u"Невозможно открыть документ: " + os.path.basename(path))
    ...
    if is_changed:
        ...
        try:
            document_xlsinput.saveAs((mydirs_[0] + "\\" + filename_err).encode('utf-8'))
            workstatuspanel.Text = u'Сохраняю файл с ошибкой:' + filename_err
        except Exception as e:
            raise Exception(u"Невозможно сохранить документ: " + (mydirs_[0] + "\\" + filename_err))
        os.remove(path)
    else:
...
def write_table(all_str_lst, worker, datetime1_end, n_rows):
    print u"Запись данных."
    current_row = n_rows
    column = list_xls("AB")
    for str_ in all_str_lst:
        index = current_row - 3  # 1
        #print u"Сохраняю в таблицу номер записи: " + str(index)
        worker.ReportProgress(93, u"Сохраняю в таблицу номер записи: " + str(index))
        ...
        # print u"str", str_[0]
        for s in str_[:-4]:
            # print u"Столбец ", column, " Строка ", row_str
            # двигаемся построчно
            if k == 4 or k == 6:
                k += 1
...
def set_cells_format(number_rows, worker,n_rows):
    ...
    print u"Применение форматирования."
    ...
    worker.ReportProgress(97, u"Формат Date для столбца F")
...
def main_(worker, folderName, mydirs_, date_end,workstatuspanel):
    ...
    try:
        ...
        raise Exception(u"Невозможно открыть документ: " + os.path.basename(mydirs_[11]))
    ...
    if len(filtered_not_prep) == 0:
        ...
        for filename in filtered_not_prep:
            ...
            dict_str = extract_txt_doc(path, folderName, mydirs_, lena,log,workstatuspanel)  # folderName - Обработка Анкет
    ...
    if rows_c < 1:
        ...
        if A4_empty and rows_c == 1:  # без вставки если пусто и 1 запись
            ...
            table_output_xlsx.insertRowAfter(n_rows - 1, copyRowStyle=True,
