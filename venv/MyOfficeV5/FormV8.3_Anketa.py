...
from ctypes import *
import os,re, sys, time, clr, datetime, \
    tkFileDialog, Tkinter, shutil,tkMessageBox,threading
from MyOfficeV8_3_TestRename import main_
from Results_v3_4 import main_results
...
textboxBrowse.Text = path_dir_root+'\\'+u'Анкеты'
mydir=os.path.dirname(textboxBrowse.Text)+'\\'+u'Обработка Анкет'
...
def FileDialog(title):
    ...
    folderName = tkFileDialog.askdirectory(initialdir="/", mustexist=1,
                                           title=title)
    return folderName
...
def message_warn(title,Message):
    root.iconbitmap(path_dir_root+"\\images\\" +"folder.ico")
    root.attributes("-topmost", True)
    dialog_result=tkMessageBox.askyesno(title, Message)
    formConvert.Focus()
...
def do_work(sender, event):
    ...
    if combobox1.SelectedIndex == 0:
        dir_list_xls = os.listdir(mydirs_[6])
        _xlsx = [i for i in dir_list_xls if re.search('.xl[xs]', i)]
        if (_xlsx): #Проверка существуют ли вообще файлы xlsx xls в папке Анкеты
            if not (os.path.exists(mydir)): # Существует ли папка Обработка Анкет
                try:
            ...
            raise Exception('Не найдены файлы xls или xlsx!')
        if os.path.exists(mydirs_[11]):
            dialog_result = message_warn('Добавить?','Вы хотите добавить информацию в : ' + os.path.basename(
                                                         mydirs_[11]))
...
def splash_form():
    ...
    splashForm.Text = 'Загрузка модулей...'.decode('utf-8')
...
def show_dialog(sender, event):
    ...
    if folderName:
        ...
        textboxBrowse.Text = os.path.normpath(folderName)
        if not re.search(u'Анкеты', textboxBrowse.Text):
            textboxBrowse.Text= os.path.normpath(textboxBrowse.Text + u'\Анкеты')
        for i in (0, 1, 2, 6, 11, 12, 13, 14, 17, 19, 20, 21):
            if i == 6:
                    mydirs_[i] = textboxBrowse.Text
            else:
                mydirs_[i] = os.path.normpath(mydirs_[i].replace(os.path.dirname(mydir),os.path.dirname(textboxBrowse.Text)+"\\"))
        ...
        mydir = os.path.normpath(os.path.dirname(textboxBrowse.Text)+u'\Обработка Анкет')
        for k,i in enumerate(mydirs_):
            print k,i
    else:
...
def show_form():
    ...
    textboxBrowse.Size = Size(310, 30)
    textboxBrowse.Multiline = False
...
if not (os.path.exists(path_dir_root+'\\images\\')):
    message_box('Обратитесь к разработчикам!!','Отсутствует папка: images')
    raise Exception(u"Отсутствует папка: images")
elif not (os.path.exists(path_dir_root+'\\images\\Document-icon.jpg')):
    message_box('Обратитесь к разработчикам!!','Отсутствует файл: Document-icon.jpg')
    raise Exception(u"Файлы отстуствуют")
elif not (os.path.exists(path_dir_root+'\\images\\folder.ico')):
    message_box('Обратитесь к разработчикам!!','Отсутствует файл: folder.ico')
    raise Exception(u"Файлы отстуствуют")
elif not (os.path.exists(path_dir_root+'\\images\\Open-folder-full.png')):
    message_box('Обратитесь к разработчикам!!','Отсутствует файл: Open-folder-full.png')
    raise Exception(u"Файлы отстуствуют")
elif not (os.path.exists(path_dir_root+'\\images\\post_logo.png')):
    message_box('Обратитесь к разработчикам!!','Отсутствует файл: post_logo.png')
    raise Exception(u"Файлы отстуствуют")


t1 = threading.Thread(target=splash_form,)
